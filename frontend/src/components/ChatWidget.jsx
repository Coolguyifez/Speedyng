...
useEffect(() => {
  ...
  const handleSendMessage = async () => {
    ...
    if (user) await carAPI.saveChatMessage({ ...userMessage, userId: user.id });

    setTimeout(async () => {
      const botReplyText = getLegitResponse(currentInput);
      const botResponse = {
        text: botReplyText,
        sender: 'bot',
        timestamp: new Date()
      ...
      // Save Bot reply to DB if logged in
      if (user) await carAPI.saveChatMessage({ ...botResponse, userId: user.id });
    }, 1000);
  };
